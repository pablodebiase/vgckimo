!  MATHS - Math Library for GLOFEKIN
...
! multiply matrix
subroutine matmult(n,m,b,a,x)
implicit none
integer*4 n,m,i,j
real*8 b(n),x(m),a(n*m),aa(n,m)
do i=0,n-1
  do j=0,m-1
    aa(i+1,j+1)=a(i+n*j+1)
  enddo
enddo
b=matmul(aa,x)
end subroutine
...
implicit none
integer*4 n,m,i,j
real*8 b(n),x(m),a(n*m),ainv(m,n),aa(n,m)
do i=0,n-1
  do j=0,m-1
    aa(i+1,j+1)=a(i+n*j+1)
  enddo
enddo
call pinv(n,m,aa,ainv)
x=matmul(ainv,b)
end subroutine
